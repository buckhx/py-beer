...
from twisted.internet.error import DNSLookupError,ConnectionRefusedError, TimeoutError, ConnectError
...
def loadBreweries(mongo):
    q = {'type':{'$in':['micro','regional','large']},'endpoints':{'$exists':False},'website':{'$ne':None}}
    print "%d Breweries queued" % mongo.find(q).count()
    return mongo.find(q,fields=['website','name'])

def agentErrback(failure,brewery):
    reason = failure.trap(DNSLookupError,ResponseFailed,ConnectionRefusedError,TimeoutError,ConnectError, SchemeNotSupported)
...
def socialRequest(brewery,agent,mongo):
    if not brewery['website'].startswith('http'):
        brewery['website'] = 'http://'+brewery['website']
    url = str(brewery['website'])
